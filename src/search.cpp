...
int AspirationWindowSearch(int prev_eval, int depth, ThreadData* td) {
    ...
    td->RootDepth = depth;
    SearchData* sd = &td->sd;
    SearchStack stack[MAXDEPTH + 6], *ss = stack + 6;
...
int Negamax(int alpha, int beta, int depth, const bool cutNode, ThreadData* td, SearchStack* ss) {
    ...
    const ZobristKey ttKey = GetTTKey(pos);
    const bool ttHit = !excludedMove && ProbeTTEntry(ttKey, &tte);
    ...
    // get an evaluation of the position:
        ...
        StoreTTEntry(ttKey, NOMOVE, SCORE_NONE, rawEval, HFNONE, 0, pvNode, ttPv);
    }
    ...
    if (!pvNode
        ...
        if (   depth < 10
            ...
            return (eval - futilityMargin(depth, improving, badNode) + beta) / 2;
        ...
        && (ttScore == SCORE_NONE || ttDepth < depth - 3 || ttScore >= pcBeta))
    {
        ...
        while ((move = NextMove(&mp, true)) != NOMOVE) {
            ...
            if (pcScore >= pcBeta) {
                StoreTTEntry(ttKey, MoveToTT(move),
    ...
    while ((move = NextMove(&mp, skipQuiets)) != NOMOVE) {
        ...
        const int moveHistory = GetHistoryScoreSearch(pos, sd, move, ss, false);
        if (   !rootNode
            && !isMated(bestScore)) {
    ...
    if (!excludedMove) {
        StoreTTEntry(ttKey, MoveToTT(bestMove), ScoreToTT(bestScore, ss->ply), rawEval, bound, depth, pvNode, ttPv);
    }
...
int Quiescence(int alpha, int beta, int depth, ThreadData* td, SearchStack* ss) {
    ...
    const ZobristKey ttKey = GetTTKey(pos);
    const bool ttHit = ProbeTTEntry(ttKey, &tte);
    const int ttScore = ttHit ? ScoreFromTT(tte.score, ss->ply) : SCORE_NONE;
    const Move ttMove = ttHit ? MoveFromTT(pos, tte.move) : NOMOVE;
    const uint8_t ttBound = ttHit ? BoundFromTT(tte.ageBoundPV) : uint8_t(HFNONE);
    ...
    if (inCheck) {
        ...
        if (ttHit) {
            ...
            auto correction = GetCorrHistAdjustment(pos, sd, ss);
            bestScore = ss->staticEval = adjustEval(pos,correction,  rawEval);
            StoreTTEntry(ttKey, NOMOVE, SCORE_NONE, rawEval, HFNONE, 0, false, ttPv);
        }
    ...
    StoreTTEntry(ttKey, MoveToTT(bestmove), ScoreToTT(bestScore, ss->ply), rawEval, bound, 0, pvNode, ttPv);
