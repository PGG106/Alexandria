...
void SearchPosition(int startDepth, int finalDepth, ThreadData* td, UciOptions* options) {
    ...
    for (int currentDepth = startDepth; currentDepth <= finalDepth; currentDepth++) {
        ...
        if (td->id == 0) {
            ...
            if (std::abs(score - averageScore) < stabilityFactor()) {
...
int Negamax(int alpha, int beta, int depth, const bool cutNode, ThreadData* td, SearchStack* ss) {
    ...
    if (!pvNode
        ...
        if (   depth < 10
            ...
            && eval - rfpRed() * (depth - improving - canIIR) >= beta)
            return eval - rfpRed() * (depth - improving - canIIR);
        ...
        if (   eval >= ss->staticEval
            && eval >= beta
            && ss->staticEval >= beta - nmpStaticevalRedDepth() * depth + nmpStaticevalRedFixed()
            && (ss - 1)->move != NOMOVE
            && depth >= 3
            && ss->ply >= td->nmpPlies
            ...
            ss->move = NOMOVE;
            const int R = 3 + depth / 3 + std::min((eval - beta) / nmpDepthRedRatio(), 3);
        ...
        // Razoring
        if (depth <= 5 && eval + rzrDepthMul() * depth < alpha)
        {
    ...
    while ((move = NextMove(&mp, skipQuiets)) != NOMOVE) {
        ...
        if (   !rootNode
            ...
            const int lmrDepth = std::max(0, depth - reductions[isQuiet][std::min(depth, 63)][std::min(totalMoves, 63)] + moveHistory / lmrDepthHistDiv());
            ...
            if (!skipQuiets) {
                ...
                if (!inCheck
                    && lmrDepth < 11
                    && ss->staticEval + fpFixed() + fpScale() * lmrDepth <= alpha) {
                    skipQuiets = true;
                }
            }
        ...
        if (totalMoves > 1 + pvNode && depth >= 3 && (isQuiet || !ttPv)) {
            ...
            if (isQuiet) {
                ...
                // Decrease the reduction for moves that have a good history score and increase it for moves with a bad score
                depthReduction -= moveHistory / lmrNoisyHistDiv();
            }
            ...
            if (score > alpha && newDepth > reducedDepth) {
                ...
                const bool doDeeperSearch = score > (bestScore + DDFixed() + 2 * newDepth);
...
int Quiescence(int alpha, int beta, ThreadData* td, SearchStack* ss) {
    ...
    while ((move = NextMove(&mp, !inCheck || bestScore > -MATE_FOUND)) != NOMOVE) {
        ...
        if (    bestScore > -MATE_FOUND
            && !inCheck) {
            const int futilityBase = ss->staticEval + QSFpFixed();
            if (futilityBase <= alpha && !SEE(pos, move, QSFpSEE())) {
