    ...
    for (int i = 0; i < moveList->count; i++) {
        ...
        // start sorting from the promotions based on the promoted piece type.
         if (isPromo(move)) {
            ...
            const int capturedPiece = isEnpassant(move) ? PAWN : GetPieceType(pos->PieceOn(To(move)));
            ...
            const int SEEThreshold = mp->SEEThreshold != score_none ? mp->SEEThreshold : -moveList->moves[i].score / 64;
