...
void NNUE::init(const char* file)
{
	...
	if (nn)
	{
		fread(featureWeights, sizeof(int16_t), INPUT_WEIGHTS * HIDDEN_SIZE, nn);
		fread(featureBias, sizeof(int16_t), HIDDEN_SIZE, nn);
		fread(outputWeights, sizeof(int16_t), HIDDEN_SIZE * 2, nn);
		fread(&outputBias, sizeof(int16_t), 1, nn);
