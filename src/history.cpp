...
void updatePHScore(const Position* pos, SearchData* sd, const Move move, int bonus) {
    ...
    // Update move score
    sd->pawnHist[PieceTo(move)][pos->state.pawnKey % PH_SIZE][CountBits(GetPieceBB(pos,PAWN))] += scaledBonus;
}
...
int GetPHScore(const Position* pos, const SearchData* sd, const Move move) {
    return sd->pawnHist[PieceTo(move)][pos->state.pawnKey % PH_SIZE][CountBits(GetPieceBB(pos,PAWN))];
}
