...
static inline Bitboard PawnPush(int color, int sq) {
	if (color == WHITE) return (1ULL << (sq - 8));
	return (1ULL << (sq + 8));
}
...
static inline void AddPawnMove(const S_Board* pos, const  int from, const int to, S_MOVELIST* list) {//function that adds a capture move of a white pawn
	...
	if (pos->side == WHITE) {
		...
		if (from >= a7 && from <= h7) { //if the piece is moving from the 7th to the 8th rank
			...
			AddMove(pos, encode_move(from, to, WP, WQ, 0), list);
			AddMove(pos, encode_move(from, to, WP, WR, 0), list); //consider every possible piece promotion
			AddMove(pos, encode_move(from, to, WP, WB, 0), list);
			AddMove(pos, encode_move(from, to, WP, WN, 0), list);
		}
		else { //else do not include possible promotions
			AddMove(pos, encode_move(from, to, WP, 0, 0), list);
		}
		...
		if (from >= a2 && from <= h2) { //if the piece is moving from the 2nd to the 1st rank
			...
			AddMove(pos, encode_move(from, to, BP, BQ, 0), list);
			AddMove(pos, encode_move(from, to, BP, BR, 0), list); //consider every possible piece promotion
			AddMove(pos, encode_move(from, to, BP, BB, 0), list);
			AddMove(pos, encode_move(from, to, BP, BN, 0), list);

		}
		else { //else do not include possible promotions
			AddMove(pos, encode_move(from, to, BP, 0, 0), list);
		}
...
static inline Bitboard LegalPawnMoves(S_Board* pos, int color, int square) {
	...
	if (pos->pinD & (1ULL << square)) return pawn_attacks[color][square] & pos->pinD & pos->checkMask & enemy;
	...
	if (pos->pinHV & (1ULL << square)) return push & pos->pinHV & pos->checkMask;
	...
	if (pos->checkMask != 18446744073709551615ULL && pos->enPas != no_sq && attacks & (1ULL << pos->enPas) && pos->checkMask & (1ULL << (pos->enPas + offset))) return (attacks & (1ULL << pos->enPas));
	// If we are in check we can do all moves that are on the checkmask
	if (pos->checkMask != 18446744073709551615ULL) return ((attacks & enemy) | push) & pos->checkMask;
	...
	if (pos->enPas != no_sq && square_distance(square, pos->enPas) == 1 && (1ULL << pos->enPas) & attacks) {
		...
		if (!((get_rook_attacks(kSQ, pos->occupancies[2]) & (pos->bitboards[(color ^ 1) * 6 + 3] | pos->bitboards[(color ^ 1) * 6 + 4]))))
...
static inline Bitboard LegalKnightMoves(S_Board* pos, int color, int square) {
	...
	if (pos->pinD & (1ULL << square) || pos->pinHV & (1ULL << square)) return NOMOVE;
	return knight_attacks[square] & ~(pos->occupancies[pos->side]) & pos->checkMask;
}


static inline Bitboard LegalBishopMoves(S_Board* pos, int color, int square) {

	if (pos->pinHV & (1ULL << square)) return NOMOVE;
	if (pos->pinD & (1ULL << square)) return get_bishop_attacks(square, pos->occupancies[BOTH]) & ~(pos->occupancies[pos->side]) & pos->pinD & pos->checkMask;
	return get_bishop_attacks(square, pos->occupancies[BOTH]) & ~(pos->occupancies[pos->side]) & pos->checkMask;
...
static inline Bitboard LegalRookMoves(S_Board* pos, int color, int square) {
	...
	if (pos->pinD & (1ULL << square)) return NOMOVE;
	if (pos->pinHV & (1ULL << square)) return get_rook_attacks(square, pos->occupancies[BOTH]) & ~(pos->occupancies[pos->side]) & pos->pinHV & pos->checkMask;
	return get_rook_attacks(square, pos->occupancies[BOTH]) & ~(pos->occupancies[pos->side]) & pos->checkMask;
	...
	if (pos->checks < 2) {
		...
		while (knights) {
			...
			while (moves) {
				...
				AddMove(pos, encode_move(source_square, target_square, piece, 0, 0), move_list);
				clr_bit(moves, target_square);
			}
			...
			if (pos->castleperm & WKCA)
			{
				...
				if (!get_bit(pos->occupancies[BOTH], f1) && !get_bit(pos->occupancies[BOTH], g1))
				{
					...
					if (!is_square_attacked(pos, e1, BLACK) && !is_square_attacked(pos, f1, BLACK) && !is_square_attacked(pos, g1, BLACK))
						AddMove(pos, encode_move(e1, g1, WK, 0,  1), move_list);
				}
			}
			...
			if (pos->castleperm & WQCA)
			{
				...
				if (!get_bit(pos->occupancies[BOTH], d1) && !get_bit(pos->occupancies[BOTH], c1) && !get_bit(pos->occupancies[BOTH], b1))
				{
					...
					if (!is_square_attacked(pos, e1, BLACK) && !is_square_attacked(pos, d1, BLACK) && !is_square_attacked(pos, c1, BLACK))
						AddMove(pos, encode_move(e1, c1, WK, 0,  1), move_list);
				}
			}
			...
			if (pos->castleperm & BKCA)
			{
				...
				if (!get_bit(pos->occupancies[BOTH], f8) && !get_bit(pos->occupancies[BOTH], g8))
				{
					...
					if (!is_square_attacked(pos, e8, WHITE) && !is_square_attacked(pos, f8, WHITE) && !is_square_attacked(pos, g8, WHITE))
						AddMove(pos, encode_move(e8, g8, BK, 0,  1), move_list);
			...
			if (pos->castleperm & BQCA)
			{
				...
				if (!get_bit(pos->occupancies[BOTH], d8) && !get_bit(pos->occupancies[BOTH], c8) && !get_bit(pos->occupancies[BOTH], b8))
				{
					...
					if (!is_square_attacked(pos, e8, WHITE) && !is_square_attacked(pos, d8, WHITE) && !is_square_attacked(pos, c8, WHITE))
						AddMove(pos, encode_move(e8, c8, BK, 0,  1), move_list);
...
void generate_captures(S_MOVELIST* move_list, S_Board* pos)
{
	...
	if (pos->checks < 2) {
		...
		while (knights_mask) {
			...
			Bitboard moves = LegalKnightMoves(pos, pos->side, source_square) & (pos->occupancies[pos->side ^ 1]);
		...
		while (bishops_mask) {
			...
			Bitboard moves = LegalBishopMoves(pos, pos->side, source_square) & (pos->occupancies[pos->side ^ 1]);
		...
		while (rooks_mask) {
			...
			Bitboard moves = LegalRookMoves(pos, pos->side, source_square) & (pos->occupancies[pos->side ^ 1]);
		...
		while (queens_mask) {
			...
			Bitboard moves = LegalQueenMoves(pos, pos->side, source_square) & (pos->occupancies[pos->side ^ 1]);
